...
describe('ZTrace', function() {
  it('Can hook into an object and watch its calls', function() {
    ...
    assert.strictEqual(infos[0][1].newTarget, null);
  ...
  it('Hooks into an return values of functions', function() {
    ...
    assert.strictEqual(infos[2][1].newTarget, null);
  ...
  it('Can display new.target', function() {
    class A {
      constructor() {
        this.a = 42;
      }
    ...
    class B extends ztrace.hookValue('A', A) {}
    ...
    assert.strictEqual(infos[1][0], 'leave');
    assert.strictEqual(infos[0][1], infos[1][1]);
    assert.strictEqual(infos[0][1].newTarget, B);
    assert.strictEqual(infos[0][1].name, 'A');
    assert.strictEqual(infos[0][1].localName, 'new A');
    assert.strictEqual(infos[0][1].thisArg, null);
    assert.strictEqual(infos[0][1].depth, 0);
    assert.strictEqual(infos[0][1].callID, 0);
    assert.strictEqual(infos[0][1].exception, undefined);
  });
