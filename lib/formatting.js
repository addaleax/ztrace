...
function formatArgs(args, pretty, targetLength = 37) {
  const formatted = args.map(value => {
    ...
    return inspected.length <= targetLength ? inspected :
        `${inspected.slice(0, targetLength-7)}...${inspected.slice(-7)}`;
  });
