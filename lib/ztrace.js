  ...
  setupHooks() {
    ...
    if (this.options.trace.binding) {
      for (let lib in libs.bindings) {
        this._hookValue(`@${lib}`, libs.bindings[lib], Object.assign({
          type: 'binding'
    ...
    if (this.options.trace.global) {
      for (let key in global) {
        ...
        const newValue = this._hookValue(key, global[key], Object.assign({
          type: 'global'
        }, this.baseContext));
    ...
    if (this.options.trace.module) {
      for (let lib in libs.modules) {
        this._hookValue(lib, libs.modules[lib], Object.assign({
          type: 'module'
        }, this.baseContext))[this.kNameOverride] = lib;
      }
    }
